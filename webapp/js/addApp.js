 ...
 */
define(
    'entries/service/addApp',['jquery', 'Class', 'entries/util/util', 'template', 'fileUpload'],
    function ($, Class, Util, template,WebUploader) {
        var addApp = Class.create(
            {
                    ...
                    uploader.on('uploadSuccess', function (file, response) {
                        if(response.responseCode==1000){
                            if($(item).hasClass('btn_file_je')){
                                var img='<div class="edit-img"><img class="file_btn" src="'+response.data.imgUrl+'" alt=""/><div class="btn-del">删除</div></div>',
...
require(
    ['jquery','entries/util/base','entries/service/addApp'],
    function ($,Base, addApp) {
        $(function () {
            Base.init(function () {
                new addApp();
            });
...
define("entries/action/addApp.js", function(){});
